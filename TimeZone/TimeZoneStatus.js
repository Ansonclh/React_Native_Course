...
// Helper function to ensure valid string rendering, avoid rendering issue
...
const TimeZoneStatus = () => {
  ...
  useEffect(() => {
    const fetchWeatherData = async () => {   //API fetching, so use async to get data before rendering
      const cities = ['Kolkata', 'Paris', 'New York', 'London', 'Tokyo', 'Sydney'];
      const weatherUpdates = await Promise.all(   //3 States of Promise: Pending (initial state), resolved, rejected => Use for multiple async operations
